...
(define ASCII-MODE 0)
(define GRAPHICS-MODE 1)

(define CELL-WIDTH 80)
(define CELL-PADDING 5)

(define FRAMES-PER-SEC 2)
...
; SYNTAX
(define-syntax try ; (http://stackoverflow.com/questions/1557854/how-does-pltscheme-catch-errors)
...
(ACTOR-IMAGE-MAPPINGS 'put "Flower" (read-bitmap "images/Flower.gif"))
...
; ACTOR WORLD
(define (new-actor-world grid [type ASCII-MODE])
...
;   #     # #     # #     #  #   #  
;   #     #  #####   #####  ### ### 
  (define (step-ascii) ; updates grid data
    ((grid 'get-occupied-locations) 'for-each (λ (loc) ((grid 'get loc) 'act)))
    (show-ascii)
    (display "\ntest\n")
    (wait (/ 1 FRAMES-PER-SEC))
    (step-ascii))
  (define (show-ascii) ; updates grid visuals
                    (display "\n\n\n\n")
    (for-each (lambda (row) 
                (display "\n")
                (for-each (λ (col)
                            (let ((loc (new-location row col)))
                              (cond ((null? (grid 'get loc)) (display EMPTY-SPOT))
                                    (else (print ((grid 'get loc) 'to-shape))))))
                          (range (grid 'get-num-cols))))
              (range (grid 'get-num-rows))))
...
;    #     # #    #  #     # #       #     #  #  #     # #     # 
;     #####  #     # #     # #       #     # ###  #####   #####  
  (define (step-graphics t) ; updates grid data
    (cond ((= (modulo (modulo t 28) (round (/ 28 FRAMES-PER-SEC))) 0) ; TODO only works well for numbers that are factors of 28
           ((grid 'get-occupied-locations) 'for-each (λ (loc) ((grid 'get loc) 'act))))) ; TOOD will probably not work for FRAMES-PER-SEC > 28
    (show-graphics))
  (define (show-graphics) ; updates grid data
    ; HELPER FUNCTIONS
    ...
    (let ((g (make-grid ROWS COLS CELL-WIDTH)))
      (define (recur row col)
        (cond ((< col 0) (recur (- row 1) (- (grid 'get-num-cols) 1)))
              ;((< row 0) (empty-scene APPLET-WIDTH APPLET-HEIGHT))
              ((>= row 0)
               (let ((loc (new-location row col)))
                 (let ((obj (grid 'get loc)))
                   ...
                   (recur row (- col 1)))))))
      ;(place-image BLANK (* (loc 'get-row) 30) (* (loc 'get-col) 30) (recur row (- col 1))))
      ;(else (place-image (obj 'to-shape) (* (loc 'get-row) 30) (* (loc 'get-col) 30) (recur row (- col 1))))))))))
  ...
  (define (play . args)
    (cond ((equal? type ASCII-MODE) (apply step-ascii args))
          (else (animate step-graphics))))
  (define (to-string)
    (grid 'to-string))
  (define (dispatch m . args)
...
(define (new-actor [name "Actor"])
  ...
  (define (act)
    ...
    (define (act-bug)
      ...
      (cond ((and (my-grid 'is-valid? loc) (null? (my-grid 'get loc))) (move-to loc))
...
;(define a (new-array))
;(define a2 (new-2d-array 10 20))
;(define l (new-location 3 4))
;(define m (new-map))
(define grid (new-grid ROWS COLS))
(define g grid)
;(g 'get-occupied-locations)
(define aw (new-actor-world g GRAPHICS-MODE))
...
(define (run-tests)
  ...
  (let ((category "HELPER"))
    ...
    (cond ((error? (λ (wd) (pluralize wd 1)) '(dog dog) '("dog" "dog") '(0 0) (let ((arr (new-array))) (list arr arr)))
  ...
  (let ((category "ARRAY"))
    (cond ((let ((arr (new-array)))
             ...
             (not (all (list
                        ...
                        (not (arr 'contains? 4))))))
           (make-error 1 category)))
    (cond ((not (exception? (λ () ((new-array) 'get 2))))
           (make-error 2 category)))
    (cond  ((not (exception? (λ () ((new-array) 'set 1 2))))
            (make-error 3 category)))
    (cond  ((exception? (λ () ((new-array) 'set 0 1)))
            (make-error 4 category)))
    (cond  ((exception? (λ () (let ((arr (new-array)))
                                     (arr 'add 1)
                                     (arr 'get 0))))
           (make-error 5 category))))
