...
(require 2htdp/universe 2htdp/image racket/draw (lib "13.ss" "srfi"))
...
(define (new-2d-array rows cols . rest)
  ...
  (define (for-each-row-iterator fn)
    (array 'for-each (λ (row) (fn row))))
  (define (for-each-iterator fn)
    (array 'for-each (λ (row) (for-each fn (row 'get-array)))))
  (define (to-string)
    (array 'to-string))
  (define (dispatch m . args)
...
(define (new-location r c)
  ...
  (define (get-adjacent-location dir)
    (let ((d (modulo dir FULL-CIRCLE)))
      (new-location
       (cond ((or (= d SOUTHEAST) (= d SOUTH) (= d SOUTHWEST)) (+ r 1))
             ((or (= d EAST) (= d WEST)) r)
             (else (- r 1)))
       (cond ((or (= d NORTHEAST) (= d EAST) (= d SOUTHEAST)) (+ c 1))
             ((or (= d NORTH) (= d SOUTH)) c)
             (else (- c 1))))))
  (define (equals loc)
    (= (compare-to loc) 0))
  (define (to-string)
...
(define (new-grid rows cols)
  ...
  (define (get-neighbors loc)
    (let ((neighbors (new-array-list))
          (x (loc 'get-row))
          (y (loc 'get-col)))
      (for-each (λ (row) 
                  (for-each (λ (col)
                              (let ((loc (new-location row col)))
                                (cond ((and (not (and (= row x) (= col y)))
                                            (is-valid? loc)
                                            (not (null? (get loc))))
                ...
                (list (- x 1) x (+ x 1)))
      neighbors))
  (define (get-empty-adjacent-locations loc)
    (let ((locs (new-array-list))
          (x (loc 'get-row))
          (y (loc 'get-col)))
      (for-each (λ (row) 
  ...
  (define (dispatch m . args)
    (cond ((eq? m 'get-num-cols) (apply get-num-cols args))
          ...
          ((eq? m 'get-neighbors) (apply get-neighbors args))
          ((eq? m 'get-empty-adjacent-locations) (apply get-empty-adjacent-locations args))
...
(define (new-actor [name "Actor"] . args)
  ...
  (define (critter-get-actors)
    (my-grid 'get-neighbors my-loc))
  (define (critter-proccess-actors actors)
    (actors 'for-each (λ (actor)
                        (cond ((and (not (actor 'is-a? "Rock"))
                                    (not (actor 'is-a? "Critter")))
                               (actor 'remove-self-from-grid my-grid))))))
  (define (critter-get-move-locations)
    (my-grid 'get-empty-adjacent-locations my-loc))
  (define (critter-select-move-location locs)
    (let ((n (locs 'size)))
      (cond ((= n 0) my-loc)
            (else (locs 'get (random n))))))
  (define (critter-make-move loc)
    (move-to loc))
  (define (crab-critter-get-actors)
    (let ((actors (new-array-list)))
      (for-each (λ (dir)
                  (let ((loc (my-loc 'get-adjacent-location (+ my-dir dir))))
                    (cond ((grid 'is-valid? loc)
                           (let ((obj (grid 'get loc)))
                             (cond ((not (null? obj)) (actors 'add obj))))))))
                (list HALF-LEFT HALF-RIGHT))
      actors))
  (define (crab-critter-get-move-locations)
    (let ((locs (new-array-list)))
      (for-each (λ (dir)
                  (let ((loc (my-loc 'get-adjacent-location (+ my-dir dir))))
                    (cond ((grid 'is-valid? loc)
                           (let ((obj (grid 'get loc)))
                             (cond ((null? obj) (locs 'add loc))))))))
                (list LEFT RIGHT))
      locs))
  (define (crab-critter-make-move loc)
    (cond ((loc 'equals my-loc)
           (set-direction (+ my-dir
                               (cond ((= (random 2) 0) LEFT)
                                     (else RIGHT)))))
                    
          (else (critter-make-move loc))))
      ...
      (let ((loc (my-loc 'get-adjacent-location my-dir)))
        (cond ((and (< box-bug-steps side-length) (bug-can-move? loc))
              ...
              (else
               ...
               (set! box-bug-steps 0)))))
    (define (act-critter)
      (critter-proccess-actors (critter-get-actors))
      (critter-make-move (critter-select-move-location (critter-get-move-locations))))
    (define (act-crab-critter)
      (critter-proccess-actors (crab-critter-get-actors))
      (crab-critter-make-move (critter-select-move-location (crab-critter-get-move-locations))))
    (define (act-rock)
      void)
    (define (act-flower) ; find a dynamic way to darken bitmap
      (set! flower-shade (cond ((>= flower-shade FLOWER-SHADES) FLOWER-SHADES)
                                   (else (+ flower-shade 1)))))
    (cond ((null? my-grid) void) 
          ((equal? my-name "Actor") (apply act-actor args))
          ((equal? my-name "Bug") (apply act-bug args))
          ((equal? my-name "BoxBug") (apply act-box-bug args))
          ((equal? my-name "Critter") (apply act-critter args))
          ((equal? my-name "CrabCritter") (apply act-crab-critter args))
  ...
  (define (is-a? name)
    (cond ((equal? name "Actor") #t)
          (else (string-contains my-name name)))) ; eg. CrabCritter is a Critter since it contains "Critter" in its name
  (define (dispatch m . args)
...
(aw 'add (new-actor "Critter"))
(aw 'add (new-actor "CrabCritter"))
(aw 'play)